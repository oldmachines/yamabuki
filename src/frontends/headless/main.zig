    ...
    while (true) {
        ...
        if (converted) |cr| {
            if (cr.stats.split_engage_addr != 0) {
                try out.print("  split: engaged at $00:{x:0>4}; {} IO routine(s), {} math site(s) shadowed ({} in the tail flavor's multiply helper)\n", .{ cr.stats.split_engage_addr, cr.stats.split_io, cr.stats.split_math_sites, cr.stats.split_mul });
                var hi: usize = 0;
