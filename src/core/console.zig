...
const PtrBankTracker = @import("ptr_bank_tracker.zig").PtrBankTracker;

// The save-state container lives in state.zig; its constants are re-exported
// here so `core.console.state_version` and friends keep resolving.
const state = @import("state.zig");
pub const state_magic = state.state_magic;
pub const state_version = state.state_version;
pub const state_header_size = state.state_header_size;
...
pub fn Console(comptime cfg: CoreConfig) type {
    return struct {
        ...
        pub const serialize_skip = .{ "steps", "prof", "usage", "skip_render", "polled_latch", "polled_consumed", "lap_latch", "ptr" };
        ...
        /// Pointer-bank provenance scratch for the conversion generator
        /// (`ptr_bank_tracker.zig`). Zero-sized unless `cfg.profile`.
        ptr: if (cfg.profile) PtrBankTracker else void,
        ...
        inline fn stepCpu(self: *Self) void {
            ...
            if (self.usage) |u| {
                ...
                if (u.ptr_banks) |pb| self.ptr.track(&self.bus, &self.cpu.regs, pb, pc, op, m8, x8, width, u.conv_window_homes);
            }
        }
        ...
        const StateFmt = state.Format(Self, @intFromEnum(cfg.accuracy));
        /// Size of a save state for this console type; see `state.zig`.
        pub const state_size: usize = StateFmt.size;

        /// Serialize the whole machine into `out` (>= `state_size` bytes)
        /// behind a versioned header (`state.zig`). The ROM image is not
        /// saved; loading requires a console built from the same ROM.
        pub fn saveState(self: *const Self, out: []u8) usize {
            return StateFmt.save(self, out);
        }

        /// Restore a state written by `saveState` (same core version and
        /// accuracy); header validation happens before any machine state
        /// is touched. See `state.zig`.
        pub fn loadState(self: *Self, in: []const u8) StateError!void {
            return StateFmt.load(self, in);
        }
    };
}
