...
| M12 ROM patch layer | planned (soft-patch BPS/IPS at load with a hash-verified source, `--save-patched` to write the result, and a hash-keyed registry so `--auto-patch` finds the right SA-1 / FastROM patch for the cart you loaded — patches stay their authors', the repo carries the index, never the payload, and never a ROM. Yamabuki already emulates the SA-1, so a converted ROM boots today. Also: opt-in auto-FastROM (mechanically derivable, but it breaks cycle-timed code, so it is a flag and a compat list, never a default), a wider framebuffer for widescreen hacks, and — the highest-leverage part — **execution coverage, hot-routine profiles and RAM access maps**, which is the input humans actually need to author a conversion. Automatically *generating* an SA-1 conversion is not on the roadmap: those are per-game reverse engineering, not a transformation an emulator can derive) |
