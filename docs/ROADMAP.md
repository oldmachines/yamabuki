...
## Repository layout, core architecture, testing strategy

These sections moved to [`ARCHITECTURE.md`](ARCHITECTURE.md) (the module
map, the core design, the design notes, how to add a chip) and
[`TESTING.md`](TESTING.md) (every gate, the test corpora and where they
come from, how to mint goldens). This file keeps the goals, the milestone
table, the M12 plan, performance engineering, and the risks.
...
| M14 | End-user UI: the SDL app becomes a standalone player — overlay menu over the paused game, gamepads + remapping (2 players, hotplug), .srm battery saves, 8 save-state slots, PNG screenshots, hold-to-rewind, scanned ROM library, per-game overrides | Plays gamepad-only end to end; config/data in OS per-user dirs; zero new dependencies (menu is a custom-drawn 5x7 UI composited into the RGB565 frame, so CRT shaders shade it too); every slice kept all gates green | In progress — landed as a stacked PR series (#91-#97), then the in-menu ROM-folder picker (`dirpicker.zig`: a row-based folder browser, so no text widget was needed) and the FastROM generation offer on the library screen, zipped ROMs (a hand-rolled central-directory reader — `std.zip`'s buffer path does not compile in 0.16 — behind the one shared ROM loader, so the headless runner and the generator take them too) and box art (a spec-complete PNG decoder of the repo's own, four discovery rules, a settle-then-decode panel on the library screen); deferred: core dirty-page serialize for handheld-class rewind (needs core sign-off) |
...
instantly. Movies are deliberately the *only* way to drive a profiled run.
A held pad mask (`--buttons`) and resuming a save state (`--state`) both existed
briefly and were removed: neither supplies a player, so both produce a number
that reads like gameplay and is not — a held mask never taps, aims, or reacts
(on a game that edge-detects presses it changes nothing at all), and a resumed
state winds down within seconds once nothing is driving it. Keeping a cheap
approximation next to a verified one only invites the approximation.
