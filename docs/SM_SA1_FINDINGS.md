...
A controlled pair on the idle machine, same recipe, same binary: serial
verification 883 s, threaded 494 s (1.8x, bounded by the longest surface's
own tier); baselines ~820 s and the coverage pad ~840 s in both; whole build
42.3 min serial, 35.9 min threaded. Both patches byte-identical to the
shipped v78, which is the equivalence proof for the threaded verifier.
The baseline snapshot cache (`--baseline-cache <dir>`) then removes the
stock phase on a repeat recipe: a write-run of v78 took 2,156 s and a
hit-run 504 s — an 8-minute build instead of 36 — and both patches are
byte-identical to the shipped v78. The snapshot is 40 MB: the evidence
union, the surface-0 profiler state, every surface's hashes, envelopes,
audio hash, summary and stock MMIO writer set, keyed on the image, each
surface's frames, anchor and mode, the anchored states, the skip and the
mode flags, and guarded by a comptime layout hash of the raw structs.
...
| s17 | PASSES all three verification surfaces (attract modulo the same RNG episode as v66; play-death 139 diverging ticks / worst run 18 — wall-time echoes only; titleskip 6/1): the first Super Metroid image with the SA-1 running the gameplay loop end to end. 455 math sites shadowed; 8 hazards, all in S-CPU-only handlers or deferred bodies | s16's fork: Ridley's tail segments all sat on one point (OAM at `$0370` stacked, tail records at `$7E:202C+` with zero velocities) while every S-CPU-unit multiply and divide checked out against the hardware result (50 of 50 in the window). The velocities come from the trig helper at `$A9:C460`, which multiplies through the PPU's mode-7 unit (`STA $00:211B` twice, `STA $00:211C`, `LDA $00:2135`) — registers the SA-1 cannot reach, so it read zero | the math shadow grows a second unit: offsets `$40-$5B` name `$211B/$211C/$2134-6`; a `$211B` store composes M7A = value<<8 \| latch (the write-twice shape), either input recomputes the signed 16x8 product through MA/MB; long-form (4-byte) sites join the walk; the handler's long branches become BRLs (the loads dispatch crossed 127 bytes — measured as laps whose sums were the previous divide's remainder). The split test multiplies -2 x 3 through the mode-7 registers each lap |
| s18 | PASSES (play-death 77 diverging ticks, worst run 2 — better than s17's 139/18); 302 of 455 math sites direct; SHIPS as v67. Measured: the escape's gameplay lag is gone (622 slowdown frames vs stock's 621, s17's 814); per lap the SA-1 now spends 0.24 frame on the game, 0.18 in the remaining COPs (triggers), 0.04 waiting on the pump, 0.04 in the calc routines — about 0.5 frame against stock's 0.68 — and idles the other half. Every slowdown frame left on every take is a door transition or a cutscene (states `$0B`, `$1F`, `$23`): the eras the mode gate keeps on the S-CPU | s17 measured: the S-CPU's gameplay utilisation falls from 53% to 14% (Brinstar take), every remaining lag run on that take sits in a door transition (state `$0A`/`$0B`, S-CPU-native under the mode gate) — but the Ceres escape gains 193 lag frames IN gameplay, every other frame. A clocked SA-1 trace (`--trace-sa1` now prints the SA-1's clock) splits a lap: **COP math handler 0.65 frame, the game itself 0.25 frame** (stock's S-CPU: 0.68), deferred waits 0.04, calc 0.04 — the shadow's dispatch, ~95 instructions per math access, ~290 accesses per lap, is the whole overrun | direct cell sites: `--split-scpu-set` records every S-CPU instruction address run while the upper copy is mapped (the handlers, the replayed IO bodies: 1,097 addresses over all seven takes); `--wg-split-shared` hands the union to the generator, and every math site outside it that is not a trigger store becomes the same opcode against the I-RAM cell — zero overhead. Triggers (multiplier B, divisor, mode-7 inputs) keep the COP |
| s19a | the gate widened to `$08-$0C` (`--wg-split-mode 0998:08-0C`); play-death stops polling at tick 16693 — inside the first door transition | the S-CPU replayed `$88:8435` from the trampoline and crashed: the routine takes FOUR INLINE ARGUMENT BYTES after its JSL (`LDA $03,S` … `ADC #4` … `STA $03,S`), read them from the trampoline's own bytes and returned into the middle of an instruction. `$80:91A9` has the same convention with 8 bytes — and it is the reason v67 wedges at frame 40828 of the 137k-frame power-on Brinstar take, in gameplay | the convention is detected from the body's idiom; the SA-1's stub captures the caller's frame and skips the bytes on return; the replay's trampoline calls a far helper that copies the bytes into I-RAM behind a fake frame whose bank is `$00`, so the body reads them from there and its adjusted RTL lands on an RTL that pops the drain's frame. Deferred bodies now also hand their returned P back to the SA-1 caller (a `CLC; RTL` is a return value) |
| s19b | play-death gets 54 ticks further, to the END of the first door transition (frame 17320 of a transition ending at 17329), then wedges | the HDMA-object spawner (`$88:8435`) records the CALLER'S PROGRAM BANK from the frame as the object's list bank, beside the list pointer it took from the inline arguments; the fake frame said `$00`, so the object interpreter later read its list from bank `$00` (ROM) and jumped through `$FFFF` | the fake frame carries the real caller's bank: I-RAM is aliased in every code bank, so the argument bytes and the RTL after them read the same from there |
| s19c | play-death ticks through the WHOLE surface with the door transitions on the SA-1 (the first time), and the 137k-frame power-on Brinstar take no longer wedges — but the state forks for good from tick 26180 (Ridley): 14,305 diverging ticks | every math site is a COP again on this image (the S-CPU set was recorded under the `$08` gate), so the first transition ends 40 frames later than stock's (wall 17330 vs 17290); from there the RNG seed `$05E5` differs and never heals — the fork is timing-born | the S-CPU set re-recorded on s19c over eight takes (2,477 addresses, the far-pool helper included); s19d carries it |
| s19d | direct sites back (293 of 455); play-death still forks from tick 26691 — the first transition now ends 14 frames after stock's (was 40), the escape has the same shape. Per-poll pairing cannot judge this: Super Metroid polls the pad every NMI, lag frames included, so a lap-count difference in a lagging era shifts every later poll's state. A per-LAP pairing was built (`--lap-cell 05B4`, movie version 4, a tick per rising edge of the loop's NMI-wait flag) — it pairs laps one for one, and the RNG sequences are identical call for call, but wall-driven state (NMI timers) then differs wherever lap timing shifts by a frame, which it does even on v66 at razor-edge intro laps | a clocked SA-1 trace across the transition: **software divide 45%**, COP handler 25%, the game 20%, the pump waits 1%. The door transition is arithmetic-bound, not IO-bound: the level-block lookups divide constantly, and my 16-round shift-subtract cost ~400 cycles where the S-CPU's divider costs 16 | the SA-1's hardware divider for dividends below `$8000` (its dividend is signed; larger ones keep the loop); `--lap-cell` and the per-lap take stay as instruments |
| s19e | the SA-1's hardware divider. A first measurement on the per-POLL Brinstar take read slowdown 2,366 → 567 and utilisation 46% → 3%, but that run had DIVERGED: on a game that lags less, a wall-indexed take walks Samus into a death and the title screen by state entry 46 of 216 (the idle title has no lag). The honest figure is the first Ceres door transition itself (state `$0B`, measured at the state cell on every image, the SA-1's writes now clocked): **stock 160 frames, v67 160 (native), s19d 180 (SA-1, software divide), s19e 152 (SA-1, hardware divide)**; the 4x reference finishes it about 49 frames sooner still, so the SA-1 lap is not yet the floor. The per-lap take as recorded is not faithful even on stock (one entry per frame with a lap, so a two-lap frame loses one): a per-lap feed needs sub-frame delivery at the lap edge. Verification: the per-poll tier fails by design (the conversion no longer lags where stock did, and wall-driven state — NMI timers, HDMA object timers — differs at the same lap from then on); against a stock reference whose S-CPU runs 4× in the gate's eras (`--ref-overclock 4`, engaged past the boot and only while the mode cell is in range — an overclocked intro wedges on its own delay loops) the first transition still ends 49 frames after the reference's, and the Ridley fight forks from tick 25014 | the SA-1's transition lap is now bound by the remaining COP triggers (~95 instructions each) and the game's own decompression; the reference approach needs the conversion lag-free too | next: a lean trigger handler; the pure-stock lower copy with an NMI-hook mapper switch (zero native-era cost); and a verdict that scores lag removal alongside per-lap equivalence |
| s19f | the Tourian take harvested (1,818 newly covered instructions, 795 sites): the projectile handler is relocated, the doors open, and Samus reaches Metroid Room 1. MEASURED on that take (10,000 frames, both images the same inputs): **slowdown 626 frames (6.3%) on stock → 138 (1.4%) on s19f; lag frames 863 → 374; S-CPU utilisation 67% → 17%**; the two door transitions take 134 and 153 frames instead of 160 each. The runs diverge inside the room (the Metroids grab a Samus who lags less at different moments — s19f's Samus lives 3,600 frames longer), so this is a scene measurement, not an equivalence check | a late-game take at last: the Tourian save from the Dropbox `snes-saves-srm` folder (999 energy, every item; charge/wave/ice/plasma equipped) loads with `--srm` and a scripted walk (Start-mash, hold Left, jump every 90 frames, shoot every 30) reaches Metroid Room 1, where four Metroids drain Samus to death — 626 slowdown frames in 10,000 and 67% utilisation on stock. On v67 and s19e the door never opened: the plasma/wave/ice projectile handler at `$90:B103` was NEVER COVERED by any take (early-game beams take the other branch), so its `LDA $0C04,X` still reads the abandoned WRAM home and returns open bus on the SA-1 — a coverage gap, not a split bug, and one v66 shares | the take ships as `recordings/tourian-metroids.ymv` + `.start.srm` and joins the recipe as a stock cover pair. Also settled: the three published TASes (lsnes, bsnes v085 compatibility core) and a casual BizHawk 100% clear all desync on yamabuki (Ceres Ridley, and the intro respectively) — their inputs extract cleanly (lsmv `input` member, bk2 `Input Log.txt`), the emulators' lag and boot timing do not match |
| s19g | s19f plus the trigger stores as JSLs (129 of the 458 sites; the rest direct or COP). MEASURED: the first Ceres door transition **128 frames** (stock 160, s19e 152); the Metroid room 115 slowdown frames in 10,000 (stock 626, s19f 138), utilisation 17%. VERDICT on the wide gate (`0998:08-0C`): surface 1 (the 36k attract) equivalent modulo one RNG-fork episode; surface 2 (play-death, the first lagging surface) FAILS the per-poll tier from frame 2,386 — the structural fork of a lag-changing conversion, not a bug (see "Where the widened split stands") | the per-lap take format is settled as unnecessary for this game: Super Metroid polls the pad exactly once per lap, so a per-poll take already pairs laps; delivering inputs at the lap edge instead of the frame start only moves them for code that reads the pad inside a lap (a save-station prompt), and the run drifts from there. The sub-frame lap feed stays in the tree, inert unless a take names a lap cell | |

**The famous lag (s19f, 2026-09-06).** Tourian's four-Metroid room, from
the late-game save: stock drops 626 of 10,000 frames (863 lag frames,
67% S-CPU utilisation); the split image drops 138 (374 lag frames, 17%).
That is the first measurement of the SA-1 removing Super Metroid's
enemy-load slowdown rather than transition lag.

| s19h | s19g with the S-CPU's copy PURE STOCK: no anchor at all in the lower copy (its laps cost what stock's cost), an NMI hook there that maps the SA-1's copy in when the mode enters the gate's range, and the takeover mapping the stock copy back the moment the SA-1 hands the loop over. Boots (title-skip hash equals stock at frame 3,000); the Metroid room: 178 slowdown frames in 10,000 (1.8%), 274 lag frames, utilisation 15% mean. The wide gate's verdict is s19g's (surface 1 modulo one RNG fork, surface 2 the structural per-poll fork) | **the soft-reset wedge.** Recording the S-CPU set over the long stock take (137,510 polls) froze the picture at frame 21,569 — and s19g freezes at the same place (frame 21,078), so it is not the lower copy's doing. The take lets Ceres' timer run out (modes $23-$25), Samus dies ($19, $1A), and the game over ends in the game's SOFT RESET: a second entry to the boot at `$80:8462` (`LDX #$1FFF / TXS`, `LDA #$0000 / TCD`, screen off, WRAM clear) that no evidence take had ever executed. The window relocation shifts only EXECUTED TCD/TCS/TXS sites fed by an immediate (the rule that keeps a computed D refusable by name), so this entry kept D = 0 and S = $1FFF: the boot then read its direct page from the abandoned WRAM home (`[stale]` at `$80:84B8`, `$80:8764`-`$87E1`), jumped through a zero pointer to `$80:0000`, and the BRK there landed in stock's own hang loop at `$80:8573`. v67's bytes at `$80:8462` are stock's too (same relocation, same evidence): a player answering NO to "TRY AGAIN?" on the shipped v67 hangs the same way — v67 dodges it on the long take only because its Samus dies elsewhere and the replayed prompt lands on YES. The fix is evidence, not code, and the evidence has to be STOCK's: on stock the long take answers YES (no reset), so a new scripted take from the Tourian save — Samus drained by the Metroids, Down then A on the prompt, then Start through the title — does the quit; stock executes `$80:8462` on it (traced) | the quit take ships as `recordings/stock-gameover-quit.ymv` + `.start.srm` (an evidence movie and a stock cover pair; the generator's movie capacity 6 → 8); the long take joins as `stock-timeup-softreset-polls.ymv` for its Ceres time-up, ship and morph-ball coverage. s19i (wide gate) and the v68 candidate (gameplay gate) generate with both |

| s19i | s19h plus the S-CPU set re-recorded on it (2,743 addresses over nine takes; 295 direct cell sites again) and the two new takes as evidence and stock covers. MEASURED on the per-poll stock take (the same laps on every build): the five transitions 141/139/142/167/129 frames (sum 718) against v67's stock-timed 190/172/191/205/158 (916) — s19h's all-COP 1,004 shows what the direct sites buy; the Metroid room 115 slowdown frames in 10,000, 351 lag frames, 17% utilisation (stock 626/863/67%); the title skip's 15,000th frame hashes equal to stock's (the pure-stock lower copy keeps the native eras exact). The soft reset works: on the long take the game dies at 20,661, resets at 21,076 and is in the attract demo by 22,109. VERDICT the structural one again (surface 1 modulo one RNG fork, surface 2 forks per poll from frame 2,386) | the quit take, being frame-indexed, is a different game on the conversion (Samus lives, the Start mash opens the pause menu) — it is STOCK evidence, and the long per-poll take is the proof the reset path now runs | measured, not verifiable by the per-poll tier; the gameplay-gate v68 candidate carries the same image minus the transition eras |

| v68c | s19i's image with the gameplay gate (`0998:08`), the verification target. FAILED the tier on the play-death surface from frame 2,386 — where v67 passed with wall-time echoes only. Isolated by RAM diffs against stock at frame 2,400: s19e, s19f and s19g differ from stock only in the echo cell `$0641` (by one); s19h and everything after it also differ in `$05BA`/`$05BB` by one each. The NMI handler shows what they are: `$05BA` counts consecutive lag frames and `$05BB` keeps the record, and the intro's longest load is 51 lag frames on stock, 52 on s19h | **per-NMI cost is not per-lap cost.** The pure-stock lower copy's NMI hook costs ~45 cycles on EVERY frame, including the 51 frames of a single long lap, so a lag streak pays it 51 times and grows by a frame; v67's anchor pays its cost once per lap, so the same load pays once and stays at 51. A game that measures its own lag streaks (and seeds state from them) sees the first and not the second | the lower hook is gone (part 35): the anchor stays in both copies, the S-CPU's copy keeps its stock math bytes, the takeover alone maps it back — v67's flow with every s19 improvement on top. v68d regenerates on it |

| v68d = **v68, SHIPPED** | v68c's recipe on part 35 (the anchor back in both copies, no NMI hook in the S-CPU's copy). Pre-check at play-death frame 2,400: only the echo cell `$0641` differs from stock, as v67. VERIFIED BEHAVIORALLY EQUIVALENT over all eight surfaces (the attract modulo one healed RNG fork, every other surface wall-time echoes only — v67's verdict profile). The image: 23 IO routines, 329 math sites shadowed (295 direct cell accesses, 129 JSL triggers, the rest COPs), 2 inline-argument callees, 435 thunk-dispatched split sites; patch 91,049 bytes; the patched stock ROM equals the generated image byte for byte | the quit path exercised on the shipped image with a per-poll version of the quit take (`stock-gameover-quit-polls.ymv`, replays on any build): death at 11,166, the soft reset at 12,183, title, file select, and gameplay again by 14,000. MEASURED: door transitions stock-timed (five of them 916 frames on the per-poll take, v67 916) since the gate leaves them native; the Metroid room 310 slowdown frames in 10,000 (3.1%), 610 lag frames, 21% utilisation against stock's 626 / 863 / 67% — the wide-gate s19i's 115 / 351 / 17% is what offloading the transitions would add | `tests/surfaces/sm-sa1/sm-sa1-v68.bps` + `.bps.cmd` + `sm-sa1-v68.scpu.set` |

| s19j | the wide gate (`0998:08-0C`) on part 35 — s19i's recipe, the NMI hook gone. First reading, RETRACTED the same evening: `--behavioral-probe` reported PASS on the save-anchored per-poll takes (Ceres with five doors, the Tourian room, the quit take), and a v69 candidate was generated on them. Those passes were VACUOUS: neither the verifier's two machines nor the generator's surface replays loaded a take's `.start.srm` sidecar (only the profile runner and the cover harvests did), so both sides replayed the polls from a save-less power-on at the title and the attract — their wall clocks never drifted, which a shorter door would have forced. With the sidecar applied everywhere (commit below) the same probes say what play-death said: on the Ceres take the conversion forks at its FIRST SA-1-run door (tick 4,303, wall 4,622): `$05B6` and the `$05BD`-`$05C4` timers by the removed lag, the RNG `$05E5` at once, then 1,825 addresses to the end; the Tourian and quit takes fork at the Metroid room the same way. The v68 gameplay gate forks there too on the frame-indexed Tourian take (the Metroids grab a Samus who lags less) — outside its verified eras, as documented | SM's transition and enemy logic consume the frame counter and the RNG is stepped per lap of a frame-paced loop, so removing lag anywhere is a different game from that point: tick-locked equality can prove a lag-changing conversion only across stretches where stock did not lag. The wide gate has none after its first door | the wide gate stays measured, not shipped; the sidecar bug is fixed (no shipped verdict depended on it: v67's and v68's verifying surfaces are power-on takes) |

| v68 take 0001 | the first human session on v68 (`recordings/v68-take0001-polls.ymv`): three minutes of green Brinstar, then the game hung at the death. Replayed: the death sequence reaches step `$15`, sends the sound driver `$FF` and calls the engine upload at `$80:8059`, which waits for the IPL's `$BBAA` and never gets it. The APU state (`--dump-ram` now prints the SPC's PC, CONTROL and both port sets, and appends ARAM) showed the driver executing at `$FF7D` since frame 8,598 — it had crashed 2,400 frames before the death, on a `$48` that arrived in mailbox port 3 without any CPU write the `--watch` could see. `--apu-port-trace` timed that write to the S-CPU at `$82:8D60`, and the bytes there explain it: stock's `STA $2117` (the VRAM address high byte) is `STA $2177` in every patch since v66 — and `$2140-$217F` mirror the APU mailbox, so the VRAM address byte went to the sound driver as a request. Four sites, all `STA $2117` after an `AND #$FC` or `LDA #$7C` | **the dispatcher scan matched opcode-shaped immediates.** The static extension's last step scans every ROM byte for `6C/7C/FC/DC` whose operand names an ACTIVE dispatch cell and marks it an instruction start, gated only against bytes the profile read as data. `$178D` is a plain WRAM variable with one covered 16-bit store; its only "dispatcher" shapes in the whole ROM are these four immediates (`$FC $8D $17` = `JSR ($178D,X)`, `$7C $8D $17` = `JMP ($178D,X)`), two inside executed code and two in code nothing reached — enough for the cell to activate and for all four to be marked as starts, whereupon the window shift of their "operand" landed on each store's low byte. Traced by elimination: a `--walk-watch` on the four addresses showed the decode loop never reached them, and the harvest cache held no opcode flag there | four guards, each measured on the real recipe: the dispatcher-shape scan and the raw marking refuse bytes the profile executed as an operand (`flag_exec` without `flag_opcode`), a cell activates only when at least one of its dispatcher shapes sits in reached code, the decode loop refuses executed interiors, and the relocation walk refuses a site whose operand bytes carry an opcode flag of their own. The interior guards alone healed the two executed sites; the reach requirement is what heals the two unreached ones. v69 regenerates on them with the take as a stock cover pair. v66, v67 and v68 all hang at the first death (or loud room) that reaches one of the four sites |

| **v69, SHIPPED** | v68's recipe on the four guards, plus the session's take as a stock cover pair (11 instructions, 5 sites, 2 bank bytes new). All four `STA $2117` sites are stock again and no other PPU-page operand moved; the take's death reaches game over (mode `$1A` at frame 12,500, the driver alive at `$1618`). VERIFIED BEHAVIORALLY EQUIVALENT over all eight surfaces — and the attract passes on wall-time echoes alone now, without its RNG-fork excuse (12 diverging ticks of 33,436), which says the four bytes were forking that surface too. Patch 90,865 bytes; the patched stock ROM equals the generated image byte for byte; the S-CPU set is v68's | v66, v67 and v68 stay on disk as history; the player should use v69 | `tests/surfaces/sm-sa1/sm-sa1-v69.bps` + `.bps.cmd` + `sm-sa1-v69.scpu.set` |

| the MMIO gate (2026-09-07) | the tier compares game logic, so a relocation that lands on hardware is invisible to it until the game notices (the sound driver's death took 2,400 frames to surface). New gate on every generation surface: each machine records its MMIO WRITER SET — every (register, instruction address) that wrote a PPU, APU-mailbox, WRAM-port, CPU or DMA register, the mailbox's sixteen mirrors folded to one port each, the writer's bank folded to the LoROM mirror — and a conversion fails the surface when it writes a register from a site stock never wrote it from ON ANY SURFACE, unless the site is generator code (stock's `$FF` padding) or the site's bytes still read as stock's instruction in every copy of the image. Those two filters are what keep a forked take (stock code the baseline never reached) and the generator's own stubs out of the verdict; what remains is a REWRITTEN instruction on hardware — the shape of the v66-v68 bug. Checked on the session's take with a stock reference: v68 reports exactly `$2143` from `$82:8D5D` and `$82:8DA2` (the two of the four sites the take reaches), v69 reports none | the same set, exported beside the patch as `<patch>.mmio` (stock's and the conversion's pairs, plus the padding ranges), is the `--stale` of hardware for a human take: `yamabuki-headless <conv.sfc> --movie <take> --mmio-ref <patch>.mmio --mmio-stock <stock.sfc>` lists every rewritten writer no generation run ever produced; `--mmio-out` on a stock replay makes a reference without a generation | the core keeps `instr_pc` (the instruction in flight; `regs.pc` has moved past the operand by the time it writes); `Bus.mmio_writers` is null outside the verifier |

| the disassembly oracle (2026-09-07) | Gilles found P.JBoy's bank logs and the assemblable disassembly built on them (InsaneFirebat/sm_disassembly, now under `~/Dev`). Every instruction line there carries its address; the lines that do not (label operands) are placed by walking from the last addressed line. `tools/sm_disasm_oracle.py` turns that into an instruction-boundary map — 132,311 starts, 357,873 data bytes — and audits the coverage the generator walked (`--cov-out` exports the profiled union and its static extension). On the v69 recipe: the profiled union puts 58,075 of its 58,081 opcode flags on real instruction starts and THREE inside instructions (`$A0:A49B`, `$A8:9060`, `$A8:9062`), all three planted by two old conversion covers; the static extension adds 97 opcode flags INSIDE instructions and 716 ON DATA. A second script explains every byte the generator changed by the disassembly's instruction at that site: of 20,333 changed instructions in the S-CPU's copy, 20,326 are known transforms (the window shift, the bank folds the SA-1's regions impose — `$A0-$BF` to `$20-$3F`, `$C0-$DF` to `$A0-$BF`, the lifted SRAM to `$42` — math cells, planted jumps) and SEVEN are corruptions, all in bank `$B3` (Norfair's Gamet and Geega): four `LDX EnemyIndex` read `$0EB4` instead of `$0E54`, a `CLC` became `SEI`, two Geega spawn-position reads moved — plus 273 data bytes of that bank rewritten under fake instruction spans. The static walk had entered the bank's spritemaps and instruction lists as code (seeded by a pointer-literal descent into `Enemy.instList`, a DATA pointer) and marched through data planting rewrites. In every patch from v66 to v69; no take has visited Norfair | the map becomes an OPTIONAL generator input, `--code-map`: it vetoes any static decode, dispatcher mark, pointer seed or rewrite at a byte the map places inside an instruction or in data, supplies each immediate's width from the assembler suffix, and seeds the walk with every start it names — so a game with a disassembly gets its whole code relocated on proven boundaries, and a game without one keeps the heuristics. The cover merge now also refuses a cover's opcode inside a stock-profiled instruction | the audit under the map runs now; the oracle and the explainer are the acceptance test (zero interiors, zero data, zero unexplained) |

| **v70, SHIPPED** | v69's recipe under the code map. The static walk reaches 132,315 instructions (v69: 69,299), the relocation rewrites 38,309 instructions of the S-CPU's copy (20,334), 469 math sites are shadowed (329) and 954 thunks dispatch (435); patch 158,468 bytes. The oracle: zero opcode flags inside instructions, zero on data; the explainer: zero unexplained rewrites in either copy — bank `$B3`'s seven corrupted instructions and 273 data bytes are stock's again. VERIFIED BEHAVIORALLY EQUIVALENT over all eight surfaces, the MMIO gate silent; both human takes replay clean against the new writer reference. The numbers are v69's (same gate) | the first patch whose entire code is relocated on proven boundaries: what a session in Norfair, Maridia or the Wrecked Ship now meets is code rewritten on the disassembly's word rather than the walk's guess. Data classes (level pointers, enemy headers, tilesets) still need evidence, so unvisited areas can still hold stale data sites | `tests/surfaces/sm-sa1/sm-sa1-v70.bps` + `.bps.cmd` (needs `$CODEMAP` from `tools/sm_disasm_oracle.py --export`) + `.bps.mmio` + `sm-sa1-v70.scpu.set` |
...
gameplay. A wall-indexed take cannot measure it end to end (the faster game
walks Samus elsewhere); the first door transition can be measured
directly: 160 frames on stock and v67, 152 on s19e, about 103 lag-free. This image is NOT verified by
the behavioral tier, and cannot be by any tier that demands per-tick
state equality with a lagging stock: wherever stock lagged and the
conversion does not, wall-driven state differs at the same lap from
then on. The instruments this raised — `--lap-cell` per-lap takes and
the `--ref-overclock` lag-free reference — are in the tree; the verdict
that scores lag removal alongside per-lap equivalence is not.

**Where the gameplay split stands (v67, 2026-09-05).** The SA-1 runs Super
Metroid's gameplay loop end to end on the verification surfaces; the S-CPU's
gameplay utilisation falls from 53% to 14% (the Brinstar take) and from
44% to 25% on the Ceres escape. Stock Super Metroid never lags in state
`$08` on any take we hold — its slowdown lives in door transitions (the
two-of-three-frames pattern of state `$0B`) and cutscenes — so the split's
gain on these takes is headroom, not removed slowdown: a lap costs the
SA-1 about half a frame where the S-CPU needed 0.68. Removing Super
Metroid's visible slowdown means widening the split to the transition
eras, which the mode gate still hands to the S-CPU. The remaining
per-lap overhead is the trigger COPs (0.18 frame): a leaner trigger
handler, or a JSL shape for trigger stores, is the next cut.
