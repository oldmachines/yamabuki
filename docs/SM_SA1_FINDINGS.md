...
| s19j | the wide gate (`0998:08-0C`) on part 35 — s19i's recipe, the NMI hook gone. `--behavioral-probe` (stock against the image, one surface, the tier's full accounting) says where the wide gate stands surface by surface: **PASS echoes** on the Ceres per-poll take (12,357 ticks, five door transitions, worst run 2), on the Tourian Metroid-room take (9,492 ticks, the door and Samus' death, worst run 2), on the per-poll quit take (12,611 ticks: death, the soft reset, the title, a new game), and on the title skip; **FAIL** on play-death, per-poll or frame-indexed alike: clean to its first SA-1-run transition (the Ceres door into Ridley's room at wall 17,240 — the conversion is 17 frames ahead there), then the RNG seed `$05E5` diverges within 50 ticks and Ridley's fight forks and reconverges fifteen times to Samus' death. The RNG is never seeded from wall time (`$80:8537` seeds it once at boot; `$88:B44E` only byte-swaps it), so the fork is in HOW MANY LAPS that transition took, which the deferred IO of the boss-music upload makes wall-driven — the game animating through lag, the limit the tier documents | the surface kind is not the cause (per-poll play-death forks identically) and neither is the intro (part 35 clears frame 2,400 to the echo cell). What a lag-changing conversion needs is surfaces anchored past the laps whose count is wall-driven; the save-anchored per-poll takes are exactly that | the v69 candidate verifies on attract, title skip, the Ceres per-poll take, the Tourian take and the per-poll quit take; play-death becomes evidence (its Ridley fight is verified by v68's gate, where that era runs native) |
