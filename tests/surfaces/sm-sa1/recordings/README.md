...
| `stock-take0001-polls.ymv` | 12,867 polls (13,798 frames on v62) | `sm-sa1-v62.sfc`, migrated to STOCK per poll | STOCK by migration: the player's first take on the v62 conversion from the stock save (`--record --srm`), re-recorded per poll with `--repoll --repoll-poweron --srm` and replayed on the stock image with its `.start.srm` sidecar. Bank $86 projectile routines, a bank $B2 enemy, the room setup ASM writing `$7E:CD22` — the 26 stale sites of the take's garbled map and stalled door. The first **format 3** recording: one entry per controller poll, so it replays on any build (its header now carries stock's CRC and no end hashes — re-stamped 2026-09-06 so it can serve as a verifying surface; the inputs are untouched). 261 instructions, 91 sites; the door stall and the minimap it exposed closed by nets in v65/v66 (findings §4n) |
| `stock-timeup-softreset-polls.ymv` | 137,510 polls | `sm.sfc` (power-on), migrated per poll | STOCK: the per-poll twin of `stock-poweron-brinstar.ymv` (same inputs, same end hash on stock), kept because it replays on ANY build. On the s19 conversions it goes somewhere the stock replay does not: Samus dies under Ceres' timer at a different lap, the replayed prompt lands on NO, and the game's own soft reset follows — which is how the un-shifted boot entry at `$80:8462` was found (findings §10, s19h). Adds no coverage on stock; a measurement take
| `stock-gameover-quit.ymv` | 14,400 | `sm.sfc` (anchored: `.start.srm` = the Tourian save) | STOCK, scripted: the Tourian walk into Metroid Room 1 (as `tourian-metroids.ymv`), then Samus drained, Down then A on "TRY AGAIN?" — NO, go to title — and Start through the title. Stock executes the boot's second entry `$80:8462` here (42 instructions, 6 sites), the soft reset every patch up to v67 hung on. Frame-indexed, so on a lag-changing conversion it is a different game (Samus lives; the Start mash opens the pause menu): evidence for stock, not a cross-build take
| `stock-gameover-quit-polls.ymv` | 1,312 polls | `sm.sfc` (anchored: `.start.srm`), migrated per poll | STOCK by migration (`--repoll` of the quit take): the same quit, one entry per poll, so it replays on ANY build — on v68 the death, the soft reset, the title, the file select and a new game all follow (findings §10, v68). Not in the recipe; the cross-build check of the soft-reset fix
| `v68-take0001-polls.ymv` | 10,387 polls | `sm-sa1-v68.sfc` (anchored: `.start.srm` = the stock save after take 0016) | The player's first session on v68 (2026-09-06), per poll so it replays on any build: green Brinstar, three doors, a pause, and a death at frame 10,984 that HUNG the game — the death jingle's sound-engine upload waited forever for an APU that had crashed 2,400 frames earlier. Root cause in findings §10 (v68 take): the static coverage walk decoded `AND #$FC / STA $2117` from its immediate byte, the window shift turned the store into `STA $2177` — the APU mailbox mirror — and the sound driver got a stray `$48` at the first of four such VRAM-address sites. Every patch from v66 carried it. Replays on stock to a death of its own (Samus lags more there); a stock cover pair either way
| `v69-take0001-polls.ymv` | 4,527 polls | `sm-sa1-v69.sfc` (anchored: `.start.srm` = the stock save after take 0016) | The first session on v69 (2026-09-07), the one v68's take could not finish: green Brinstar, a death at frame 3,959, the whole death sequence (`$13`-`$1A`), NO at "TRY AGAIN?", the soft reset and the intro. Sync-verified on v69; zero `--stale` sites; zero MMIO offenders against `sm-sa1-v69.bps.mmio`. The human confirmation of both the sound-driver fix and the soft-reset fix; a stock cover pair for the next recipe
| `v70-take0001-polls.ymv` | 5,193 polls | `sm-sa1-v70.sfc` (`.start.srm` = the stock save after take 0016, the same start as the v68/v69 takes) | The first session on v70 (2026-09-07): green Brinstar, the destructible blocks shot open — and every revealed block drawn as a crossed tile `$3FF`, the report that produced v71 (the PLM draw list's WRAM pointer immediate at `$84:8B3B` was never shifted; findings §10, v71). Sync-verified on v70 and v71; on v71 the blocks draw as stock's and the MMIO gate is silent; `--stale` names three evidence-gated sites left on purpose (`$86:8030`, `$86:8033`, `$A0:A3B9`). Not yet in any recipe as a cover pair
| `v72-take0001-polls.ymv` | 10,734 polls | `sm-sa1-v72.sfc` (`.start.srm` = the stock save after take 0016) | The first session on v72 (2026-09-07): green Brinstar and the area map, which rendered CLEAN this time. Sync-verified on v72; zero `--stale` sites; zero MMIO offenders against `sm-sa1-v72.bps.mmio`. The clean half of the pair that isolated the area-map bug: same map as take 0002, different outcome, so the cause was per-run-state, not per-build
| `v72-take0002-polls.ymv` | 4,429 polls | `sm-sa1-v72.sfc` (`.start.srm` = the stock save after take 0016) | The second session on v72 (2026-09-08), ending ON the garbled area map — the reference case for findings §4o. Short enough that a full `--dma-trace` fits its quota, which is what named the legend's DMA source (`$40:3640 -> VRAM $3B20`) and, with frame-bisected dumps, the WMDATA-port fill that stock uses to restore the buffer. Sync-verified on v72; renders the legend byte-identical to stock, with stock's own end-frame hash, on the WMDATA-net image (v73). The regression case for that net
